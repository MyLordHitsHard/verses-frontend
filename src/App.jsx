...
import {
  fetchPoems, fetchTags, toggleLike, checkLiked,
  createPoem, updatePoem, deletePoem, adminFetchAll,
  authGoogle, authMe, setToken, getToken, clearToken,
} from "./api/index.js";

/* ─── Google OAuth helper ────────────────────────────────────────────────── */
// Renders the invisible Google Sign-In button and resolves with the idToken
...
const Icon = {
  ...
  Loader: () => <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" style={{animation:"spin .7s linear infinite"}}><path d="M21 12a9 9 0 1 1-6.219-8.56"/></svg>,
};
...
.card.feat{grid-column:1/-1;display:grid;grid-template-columns:1fr 1fr;gap:0;padding:0;background:var(--s2)}
...
export default function App() {
  ...
  const [authLoading, setAuthLoading] = useState(true); // checking stored token on mount
  ...
  /* ── On mount: restore session + fetch tags ───────────────────────────── */
  useEffect(() => {
    (async () => {
  ...
  const loadPage = useCallback(async (cursor = null, tag = "All", resetList = false) => {
    ...
    try {
      ...
      setPoems(prev => resetList ? batch : [...prev, ...batch]);
      setHasMore(more);
      setNextCursor(nc);
      if (resetList) setTotalCount(0); // will update from next tag fetch
    } catch (err) {
  ...
  useEffect(() => {
    const obs = new IntersectionObserver(
      ([entry]) => { if (entry.isIntersecting && hasMore && !loadingRef.current) loadPage(nextCursor, activeTag); },
      { threshold: 0.4 }
    );
  ...
  useEffect(() => {
    const h = e => {
      if (e.key === "Escape") { setSelected(null); setSearchOpen(false); setShowLogin(false); setShowAdmin(false); }
  ...
  const handleLogout = () => {
    clearToken(); setUser(null); showT("Signed out");
  };
  ...
  const getLikes = (poem) => likeCounts[poem.id] !== undefined ? likeCounts[poem.id] : poem.likes;
  ...
  const startEdit = (poem) => {
    setEditId(poem.id);
    setForm({ title: poem.title, content: poem.content, author: poem.author, category: poem.category || "", tags: (poem.tags || []).join(", "), is_featured: poem.is_featured || false });
  };
  ...
  const handleSave = async () => {
    ...
    try {
      if (editId) {
        await updatePoem(editId, payload);
        showT("✓ Poem updated");
      } else {
  ...
  return (
    <>
      ...
      <section className="hero">
        ...
        <div className="h-cta">
          <button className="goldBtn" onClick={() => document.getElementById("wr")?.scrollIntoView({behavior:"smooth"})}><Icon.Book />Read Writings</button>
          <button className="gBtn" style={{height:36,padding:"0 18px"}} onClick={() => setSearchOpen(true)}><Icon.Search />Search <span style={{opacity:.5,fontSize:10}}>⌘K</span></button>
        </div>
      ...
      <main id="wr">
        ...
        <div className="sh"><h2 className="sh-title">All Writings</h2></div>
        ...
        <div className="grid">
          {poems.map((poem, i) => {
            ...
            const feat = i === 0 && activeTag === "All" && poem.is_featured;
            return (
              <div key={poem.id} className={`card ${feat?"feat":""}`} onClick={() => setSelected(poem)} style={{animationDelay:`${(i % PAGE_SIZE) * 0.06}s`}}>
                {feat ? (
                  <>
                    <div className="feat-l">
                      <div>
                        <div className="feat-badge"><Icon.Star />Featured · {poem.category}</div>
                        <div className="c-title" style={{fontSize:26,marginBottom:18}}>{poem.title}</div>
                        <div className="c-meta" style={{marginBottom:0}}>
                          ...
                          <span className="c-mi"><Icon.Tag />{(poem.tags||[]).slice(0,2).join(", ")}</span>
                        </div>
                      </div>
                      <div style={{display:"flex",gap:8,alignItems:"center",marginTop:20}}>
                        <button className={`lbtn ${lk?"on":""}`} onClick={e=>handleLike(e,poem.id)}><Icon.Heart f={lk}/>{getLikes(poem)}</button>
                        <button className="iBtn" style={{width:30,height:30}} onClick={e=>copyPoem(e,poem)}><Icon.Share /></button>
                        {isAdmin && <button className="iBtn" style={{width:30,height:30}} onClick={e=>{e.stopPropagation();startEdit(poem);openAdmin();}}><Icon.Edit /></button>}
                      </div>
                    </div>
                    <div className="feat-r">
                      <div className="feat-q">"</div>
                      <div className="feat-text">{poem.content.split("\n").slice(0,7).join("\n")}…</div>
                    </div>
                  </>
                ) : (
                  <>
                    ...
                    <div className="c-foot">
                      ...
                      <div style={{display:"flex",gap:5,alignItems:"center"}}>
                        <button className={`lbtn ${lk?"on":""}`} onClick={e=>handleLike(e,poem.id)}><Icon.Heart f={lk}/>{getLikes(poem)}</button>
                        {isAdmin && <button className="iBtn" style={{width:27,height:27}} onClick={e=>{e.stopPropagation();startEdit(poem);openAdmin();}}><Icon.Edit /></button>}
                      </div>
                    </div>
                  </>
                )}
      ...
      {searchOpen && (
        <div className="sOv" onClick={() => setSearchOpen(false)}>
          <div className="sBox" onClick={e=>e.stopPropagation()}>
            <div className="sWrap">
              <input className="sInp" placeholder="Search poems & writings…" value={searchQuery} onChange={e=>setSearchQuery(e.target.value)} autoFocus/>
              <button className="sX" onClick={() => { setSearchOpen(false); setSearchQuery(""); }}><Icon.X s={18}/></button>
            </div>
            {searchResults.map(p => (
              <div key={p.id} className="sR" onClick={() => { setSelected(p); setSearchOpen(false); setSearchQuery(""); }}>
      ...
      {selected && (
        <div className="mOv" onClick={() => setSelected(null)}>
          <div className="mC" onClick={e=>e.stopPropagation()}>
            ...
            <div className="m-ey"><Icon.Tag/>{selected.category}{(selected.tags||[]).map(t=><span key={t} style={{color:"var(--t2)"}}>· {t}</span>)}</div>
            ...
            <div className="m-foot">
              ...
              <div className="m-acts">
                <button className={`lbtn ${liked.has(selected.id)?"on":""}`} onClick={e=>handleLike(e,selected.id)} style={{padding:"7px 15px"}}>
      ...
      {showLogin && (
        <div className="lOv" onClick={() => setShowLogin(false)}>
          <div className="lCard" onClick={e=>e.stopPropagation()}>
            <button className="iBtn" style={{position:"absolute",top:12,right:12,margin:0}} onClick={()=>setShowLogin(false)}><Icon.X s={14}/></button>
            <div className="lLogo">VERSES</div>
            <div className="lSub">Private admin access<br/>Restricted to authorized email only</div>
          ...
          <div className="drw" onClick={e=>e.stopPropagation()}>
            <div className="drHead">
              ...
              <div style={{display:"flex",gap:8}}>
                ...
                <button className="iBtn" style={{margin:0}} onClick={()=>{setShowAdmin(false);resetForm();}}><Icon.X s={14}/></button>
              </div>
            </div>
            <div className="drBody">
              ...
              <div className="fGrp">
                ...
                <textarea className="fTA" value={form.content} onChange={e=>setForm(p=>({...p,content:e.target.value}))} placeholder={"Write your poem here…\n\nLine breaks are preserved."} style={{minHeight:200}}/>
              </div>
              <div className="fRow">
                <div className="fGrp">
              ...
              <div className="fGrp" style={{display:"flex",alignItems:"center",gap:9}}>
                <input type="checkbox" id="ft" checked={form.is_featured} onChange={e=>setForm(p=>({...p,is_featured:e.target.checked}))} style={{accentColor:"var(--gold)",width:15,height:15}}/>
                <label htmlFor="ft" className="fLbl" style={{margin:0}}>Set as featured poem</label>
              </div>
              <button className="goldBtn" style={{width:"100%",justifyContent:"center",height:42,marginTop:6}} onClick={handleSave} disabled={saving}>
                {saving ? <><Icon.Loader />{editId?"Saving…":"Publishing…"}</> : <><Icon.Plus/>{editId?"Save Changes":"Publish Poem"}</>}
              </button>
              ...
              <div className="aList">
                ...
                {adminPoems.map(poem => (
                  <div key={poem.id} className="aItem">
                    <div style={{flex:1,minWidth:0}}>
                      <div className="aItem-t">{poem.title}</div>
                ...
                {adminPoems.length === 0 && <p style={{fontSize:12,color:"var(--t3)",textAlign:"center",padding:"20px 0"}}>No poems yet. Write your first one above.</p>}
              </div>
            </div>
          </div>
        </div>
